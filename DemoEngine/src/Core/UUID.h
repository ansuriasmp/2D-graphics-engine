...
namespace std {
	template <>
	struct hash<DemoEngine::UUID>
	{
		std::size_t operator() (const DemoEngine::UUID& uuid) const
		{
	...
	template <>
	struct hash<DemoEngine::UUID32>
	{
		std::size_t operator() (const DemoEngine::UUID32& uuid) const
		{
